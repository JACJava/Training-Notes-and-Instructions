...
2.  Create the hbfl application (EC2, LB, ASG, Kinesis, RDS database and required security) using the cloudformation template and following command from :  aws cloudformation deploy --template-file hbfl.updated.template --stack-name hbfl-stack-cf --capabilities CAPABILITY_IAM
